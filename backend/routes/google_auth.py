...
REDIRECT_URI = os.getenv('REDIRECT_URI', 'http://127.0.0.1:5000/api/auth/google/callback')
...
def google_callback():
    try:
        ...
        return redirect(
            f"{FRONTEND_URL}?token={token}&user_name={urllib.parse.quote(name)}&user_email={email}&user_id={user.id}"
        )
