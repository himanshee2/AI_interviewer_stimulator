...
function Interview({ navigate, interviewData, setInterviewData, theme, isDark, toggleTheme }) {
  ...
  const captureEmotion = async () => {
    ...
    try {
      const res = await axios.post('http://127.0.0.1:5000/api/analyze-emotion', { image: canvas.toDataURL('image/jpeg') })
  ...
  const analyzeVoiceFile = async (file) => {
    ...
    try {
      const res = await axios.post('http://127.0.0.1:5000/api/analyze-voice', formData)
  ...
  const submitAnswer = async () => {
    ...
    try {
      ...
      const res = await axios.post('http://127.0.0.1:5000/api/evaluate-answer', {
